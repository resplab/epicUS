...
This document outlines the steps taken determine input values for the payoff module. 

## Step 1: Calculating yearly costs of COPD medication

To estimate the yearly cost of COPD medication the following was used:

1. **Average monthly GoodRx cash prices**
-  Average of the most common inhalers within each drug class (2025 Costs USD)  
2. **Real-world prescription fill frequency per year** from published sources. 
...
drug_costs <- data.frame(
  class = c("ICS + LAMA + LABA", "ICS-containing", "LAMA-containing", "LABA-containing", "SABA"),
  monthly_cost = c(653, 227, 334, 420, 31),
  dispenses_per_year = c(6.1, 5.3, 5.2, 5.5, 12)
)

drug_costs$annual_cost <- round(drug_costs$monthly_cost * drug_costs$dispenses_per_year, 2)

kable(drug_costs,
      col.names = c("Drug Class", "GoodRx Monthly Cost (USD)", "Dispenses/Year", "Estimated Annual Cost (USD)"),
...
kable(copd_background_costs,
      col.names = c("GOLD Stage", "Total COPD Cost", "Inpatient", "ER Visits", "Pharmacy", "Background Cost"),
      format.args = list(big.mark = ","),
...
- **Dalal et al., 2011** (DOI: 10.1016/j.rmed.2010.09.003) — cost data in 2008 USD  
- **Bogart et al., 2020** (DOI: 10.37765/ajmc.2020.43157) — cost data in 2017 USD
...
kable(smoking_cessation,
      col.names = c("Pharmacotherapy", "Reweighted Proportion (%)", "Cost "),
